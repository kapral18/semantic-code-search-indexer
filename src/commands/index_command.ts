...
import { createIndex, deleteIndex, createSettingsIndex, updateLastIndexedCommit } from '../utils/elasticsearch';
...
export async function index(directory: string, clean: boolean, options?: IndexOptions) {
  ...
  const gitBranch =
    options?.branch ?? execFileSync('git', ['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: directory }).toString().trim();
