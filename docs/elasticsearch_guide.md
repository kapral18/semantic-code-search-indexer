  ...
  const response = await client.search({
    ...
    query: {
      sparse_vector: {
        ...
        inference_id: '.elser_model_2', // Or process.env.ELASTICSEARCH_MODEL
        query: query,
      },
    },
  });
...
*   **ELSER Model:** The `code-indexer` sets up a pipeline to use the ELSER model. Ensure that the ELSER model is available in your Elasticsearch cluster. The model ID is configurable via the `ELASTICSEARCH_MODEL` environment variable (defaulting to `.elser_model_2`).
